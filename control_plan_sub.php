...
$crud = new Database();
if(isset($_POST)){
//echo str_replace('"',' ',$_POST['date']);die;
    ...
    $data = array(
        ...
        'process_remark'  => $_POST['process_remark'],
        ...
        'report_year' => str_replace('"',' ',$_POST['report_year']),
		...
		'updated_time'=>substr($_POST['date'], 10),
        'remark_thread_hardness' => str_replace('"',' ',$_POST['remark_thread_hardness'])

     );
     //print_r($data);die;
    $result = $crud->insert('control_plan',$data);
    if($result){
        
        
        $_SESSION['msg_success'] = 'Record Added Successfully';
    	header('Location:update_control_plan?id='.$result);
    }
    else{
