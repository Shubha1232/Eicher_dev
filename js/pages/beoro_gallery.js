/* [ ---- Beoro Admin - gallery grid ---- ] */
    ...
    beoro_gal_grid = {
                ...
                $('#large_grid ul').imagesLoaded(function() {
                    ...
                    var updateFilters = function() {
                        ...
                        for(; i<length; i++) {
                            ...
                            if(li.hasClass('active')) {
                                filters.push('#large_grid ul li.'+li.data('filter'));
                            }
                        }
                        if(filters.length == 0) {
                    ...
                    $('.remove_selected_img').click(function(e) {
                        ...
                        $('#large_grid ul li.checked_item').each(function() {
                            $(this).remove();
                            beoro_gal_grid.hide_item($(this).prop('id'));
                        })
                        refresh();
                    })
                    // remove single image
                    $('.img_action_remove').on('click',function(e) {
                        ...
                        beoro_gal_grid.hide_item($(this).closest('li').prop('id'));
                        refresh();
                    });
                    // edit selected
                    ...
                    function saveInfo() {
                        $('.edit_img_save').on('click', function() {
                            ...
                            $('#'+$(this).next('.edit_img_id').val()).prop('data-folder',thisFolder).removeClass().addClass("folder_all "+thisFolder);
                    ...
                    function saveAllInfo() {
                        $('.edit_img_save_all').on('click', function() {
                            $('#edit_image_table tbody tr').not(':last').each(function() {
                                ...
                                $('#'+$(this).find('.edit_img_id').val()).prop('data-folder',thisFolder).removeClass().addClass("folder_all "+thisFolder);
                                $(this).closest('tr').remove();
                            })
                            beoro_gal_grid.close_editBox();
                            refresh();
        ...
        getEditInfo: function(item) {
            var thisImgSrc = item.find('.img_holder').children('img').prop('src');
            var thisTitle = item.find('.imgTitle').text();
            var thisFolder = item.prop('data-folder');
            var thisId = item.prop('id');
            var clonedTemplate = $('#edit_image_template').clone();
            clonedTemplate.removeAttr('id').removeAttr('style').find('img').prop('src', thisImgSrc);
            clonedTemplate.find('.edit_img_title').val(thisTitle);
            clonedTemplate.find('.edit_img_folder').val(thisFolder).prop('checked', true);
