...
<!--								<button class="btn btn-info" onclick="printfun();" style="float: right; margin-right: 5px;">Word</button>-->
                                <form class="form-horizontal" action="edit_metallurgical_report_sub.php" id="control_plan_form" method="post" enctype="multipart/form-data">
                                        ...
                                        <table class="table table-bordered">
                                            ...
                                            <tr>
                                              <td><input onkeyup="part_check(<?php echo $i; ?>),part_show_date(<?php echo $i; ?>),show_specification(<?php echo $i; ?>)" type="text" id="part_no<?php echo $i; ?>" value="<?php echo $part_no[$i] ?>" name="part_no[]" onkeyup="getForgingImage('forging_image<?php echo $i ?>','part_no<?php echo $i ?>')"><p id="forging_image<?php echo $i ?>" style="margin:5px 0px 0px;">
			  	...
			  	$queryspef = "SELECT jominy_value FROM jominy_master_list WHERE part_no='".$part_no[$i]."'";
$rsspef = $crud->getSingleRow($queryspef);
$spe_val = $rsspef["jominy_value"];
											  }
											  
											  
											  ?><input type="text" id="specification<?php echo $i; ?>" name="specification[]" value='<?php echo $spe_val; ?>'></td>
...
<td><input type="hidden" id="part_current_date" style="width: 39%;" name="part_current_date" value="<?php echo date('Y-m-d',time()); ?>" readOnly=""/></td>
											...
											<tr>
                                              <td><input onkeyup="part_check(<?php echo $p; ?>),part_show_date(<?php echo $i; ?>),show_specification(<?php echo $i; ?>)" type="text" id="part_no<?php echo $p; ?>" name="part_no[]" onkeyup="getForgingImage('forging_image<?php echo $p ?>','part_no<?php echo $p; ?>')"><p id="forging_image<?php echo $p ?>" style="margin:5px 0px 0px;"></p></td>
...
<script>
    ...
    $(add_button).click(function(e){ //on add input button click
        ...
        if(x < max_fields){ //max input box allowed
            ...
            $(wrapper).append('<tr><td><input onkeyup="part_check('+x+'),part_show_date('+x+'),show_specification('+x+')" type="text" id="part_no'+x+'" name="part_no[]" ></td><td><input type="text" id="specification'+x+'" name="specification[]"></td><td><input type="text" id="observed" name="observed[]"></td><td id="verified_by_td'+x+'"><select id="part_grade'+x+'" name="part_grade[]" style="width:140px;"><option value="">Select</option><?php $query = 'SELECT * FROM grade';$result = $crud->getAllData($query);foreach ($result as $key => $value) {?><option value="<?php echo $value['id']; ?>" ><?php echo $value['grade']; ?></option><?php } ?></select></td><td><select style="width:140px !important" id="accept" name="accept[]"><option value="">Select</option><option value="ACCEPTED">ACCEPTED</option><option value="REJECTED">REJECTED</option><option value="ACCEPT UNDER DEVIATION">ACCEPT UNDER DEVIATION</option></select></td><td ><td><input type="text" id="part_date'+x+'" name="part_date[]" required="" autocomplete="off" readOnly=""></td></tr>'); //add input box
        }
    });
