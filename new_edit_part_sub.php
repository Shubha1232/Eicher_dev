...
if (isset($_POST)) {
	...
	$query1   = "SELECT * FROM forging_drawing WHERE new_part_id='" . $part_id . "'";
    $rs1      = $crud->getSingleRow($query1);
     $q2 = "SELECT * FROM part_number_images_name WHERE new_part_id = ".$part_id;
  $res2 = $crud->getSingleRow($q2);
    ...
    foreach($_FILES["forging_image"]["tmp_name"] as $key=>$tmp_name)
		...
		$nw_query = "select * from `part_number` where `part_no`='".$rs['part_no']."' ";
		$nw_result = $crud->getSingleRow($nw_query);
		$part_id = $nw_result['id'];
		$query1   = "SELECT * FROM forging_drawing WHERE part_number='" . $rs['part_no'] . "'";
    	$rs1 = $crud->getSingleRow($query1);
     	$q2 = "SELECT * FROM part_number_images_name WHERE part_no = ".$rs['part_no'];
  		$res2 = $crud->getSingleRow($q2);
    ...
    foreach($_FILES["part_image"]["tmp_name"] as $key=>$tmp_name)
         ...
         $error1 = rtrim($error1,',');
            ...
            $file2       = explode('.', $file_name2);
            if ($file_name2 == '') {
                $file_name2 = $nw_result['part_image2'];
                $customer_drawing_image_name = $res2['customer_drawing_image_name'];
            ...
            $file3       = explode('.', $file_name3);
            if ($file_name3 == '') {
                $file_name3 = $nw_result['part_image3'];
                $part_image_name = $res2['part_image_name'];
    ...
    foreach($_FILES["forging_image"]["tmp_name"] as $key=>$tmp_name)
		...
		if($nw_result){
			$nw_data = array(
				...
				$crud->update($table, $nw_data, $where);
			}else{
				$nw_data = array(
				'part_no' => $_POST['part_no'],
				'part_id'=>$_POST['part_id'],
                ...
                'user_id' => $_SESSION['user_id']
						...
						//echo "<pre>";print_r($nw_data);die;
						
				$ins_res = $crud->insert($table, $nw_data);
				}
				if(empty($_FILES["forging_image"]["tmp_name"])){
						$data_forging   = array(
